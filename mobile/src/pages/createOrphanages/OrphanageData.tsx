...
export default function OrphanageData() {
  ...
  const params = route.params as routeParamsData;
  ...
  async function handleSelectedImages(){
    ...
    if(status !== 'granted') {
      alert('Eita! Preciso de permissão para colocar as fotos. :(');
      return;
    };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 24 }}>
      ...
      <View style={styles.uploadImagesContainer}>
        {images.map(image => {
          return(
            <Image key={image} source={{uri: image}} style={styles.uploadedImages}/>
          );
        })}
      </View>
